...
func TestEncPage(t *testing.T) {
	...
	// serialize page
	testPageSerial, err := myPage.serializePage(uint16(myPageSize), true, testKey)
	if err != nil {
		t.Error(err)
		return
	}
	...
	// deserialize again
	decodedHead, decodedBytes, err := deserializePage(testPageSerial, true, testKey)
	if err != nil {
		t.Error(err)
		return
	}
